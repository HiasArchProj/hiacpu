...
class CSR(val parameter: CSRParameter)
  ...
  val LD_LH = parameter.ctrlParameter.LD_LH
  val LD_LHU = parameter.ctrlParameter.LD_LHU
  val ST_SW = parameter.ctrlParameter.ST_SW
  val ST_SH = parameter.ctrlParameter.ST_SH
  ...
  val laddrInvalid = MuxLookup(io.ld_type, false.B)(
    Seq(LD_LW -> io.addr(1, 0).orR, LD_LH -> io.addr(0), LD_LHU -> io.addr(0))
  )
  val saddrInvalid =
    MuxLookup(io.st_type, false.B)(Seq(ST_SW -> io.addr(1, 0).orR, ST_SH -> io.addr(0)))
